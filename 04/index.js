...
function decode (message) {
  const regex = /\(([^()]+)\)/g
  const textCut = regex.exec(message)
  const textInverted = textCut[1].split('').reverse().join('')
  const messageModified = message.replace(textCut[0], textInverted);
  if(messageModified.match(regex)){
    const messageReturn = decode(messageModified);
