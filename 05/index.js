...
function cyberReindeer (road, time) {
  ...
  for (let index = 1; index < time; index++) {
    ...
    if (roadTemp.charAt(indexTruck + 1) !== '|') {
      const temp = road.charAt(indexTruck) === '|' ? '*' : '.'
      const prefix = roadTemp.substring(0, indexTruck)
      const suffix =
        roadTemp.charAt(indexTruck) + roadTemp.substring(indexTruck + 2)
      roadTemp = prefix + temp + suffix
      indexTruck++
    }
